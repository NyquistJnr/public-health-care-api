...
class IsFacilityITAdmin(permissions.BasePermission):
    """Restricts access to Facility IT Admins (role='FACILITY_IT_ADMIN'), Officers In Charge (role='OFFICER_IN_CHARGE') or Django superusers."""

    def has_permission(self, request, view):
        user = request.user
        return bool(user and user.is_authenticated and (user.is_superuser or user.role in ['FACILITY_IT_ADMIN', 'OFFICER_IN_CHARGE']))
...
class IsNurse(permissions.BasePermission):
    """Restricts access to Nurses (role='NURSE') or Django superusers."""

    def has_permission(self, request, view):
        user = request.user
        return bool(user and user.is_authenticated and (user.is_superuser or user.role == 'NURSE'))
